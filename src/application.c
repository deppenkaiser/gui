...
void _application_startup(GApplication* app, gpointer user_data)
{
...
void _application_activate(GApplication* app, gpointer user_data)
{
	if (application_callback != NULL)
	{
...
void _application_shutdown(GApplication* app, gpointer user_data)
{
	if (application_callback != NULL)
	{
...
int32_t application_run(const char* name, int argc, char **argv, void* user_data)
{
    ...
    g_signal_connect(core.app, "activate", G_CALLBACK(_application_activate), &core);
    g_signal_connect(core.app, "startup", G_CALLBACK(_application_startup), &core);
    g_signal_connect(core.app, "shutdown", G_CALLBACK(_application_shutdown), &core);
